...
source_directory_list = ['spatial_noise']

compiler_args = []
link_args = []

version = "0.01"
description="3d spatial noise sampler."
author = "Hoyt Koepke"
...
################################################################################
# First, detect if there are any cmake variables overriding things.

def find_command_line_var(name):
    for i, a in enumerate(sys.argv):
        if a.startswith(name):
            print "From CMake: ", a
            if not a.startswith(name + "="):
                raise NameError("Bad Argument: ", a)

            sys.argv.pop(i)
            return a[len(name) + 1:]
        
    raise RuntimeError("This setup.py meant to be called only from within cmake.")

def extract_parts(s):
    l = re.split(r"[\s;\"\\:]+", s)
    return [le.strip() for le in l if len(le.strip()) != 0]

compiler_args      += extract_parts(find_command_line_var("CMAKE_CXX_FLAGS"))
extra_include_dirs += extract_parts(find_command_line_var("CMAKE_INCLUDE_DIRECTORIES"))
extra_library_dirs += extract_parts(find_command_line_var("CMAKE_LINK_DIRECTORIES"))
library_includes   += extract_parts(find_command_line_var("CMAKE_LIBRARIES"))
link_args          += extract_parts(find_command_line_var("CMAKE_LINKER_FLAGS"))

print "library_includes = ", library_includes

######################################################
# Now have to see if we're authorized to use cython files, or if we
...
if __name__ == '__main__':
    ...
    def get_extra_compile_args(m):
        return strip_empty(compiler_args)
    
    def get_extra_link_args(m):
        return strip_empty(link_args)
